...
- **Regression note** — the sort UI originally lived inside `palette-paint-list.tsx` (the flat list). After [#09 color-palette-groups](./09-color-palette-groups.md) made `PaletteGroupedPaintList` the editor's default list component, the sort UI was never reachable from the editor. [#14 palette-sort-bar-missing](./14-palette-sort-bar-missing.md) lifts it into `PaletteBuilder` and retires `palette-paint-list.tsx`.
