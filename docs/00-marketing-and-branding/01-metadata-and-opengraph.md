...
- [x] `src/app/robots.ts` exposes a robots policy (allow all in production, disallow in non-prod via env)
- [x] `src/app/sitemap.ts` generates a sitemap including all public static routes plus public palettes, paints, brands, hues, and profiles
- [x] `src/app/paints/[id]/page.tsx` exports `generateMetadata` returning the paint's name + brand in the title and a description containing the paint's hex / hue
- [x] `src/app/brands/[id]/page.tsx` exports `generateMetadata` with brand name + paint count
- [x] `src/app/hues/[id]/page.tsx` exports `generateMetadata` with hue name + Itten classification
...
- [x] Dynamic OG image route handlers exist at `src/app/api/og/paint/[id]/route.tsx`, `src/app/api/og/palette/[id]/route.tsx`, `src/app/api/og/brand/[id]/route.tsx`, `src/app/api/og/hue/[id]/route.tsx`, `src/app/api/og/user/[id]/route.tsx`, each returning a 1200×630 `ImageResponse`
- [x] Each dynamic page wires its OG image URL into `openGraph.images` and `twitter.images` via `generateMetadata`
- [x] Private palettes do NOT expose a dynamic OG image (the route handler returns 404 for non-public palettes)
