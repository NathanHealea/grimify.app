...
export default function ColorWheel({ paintGroups, searchMatchIds, isSchemeMatching }: ColorWheelProps) {
  ...
  const handleTouchMove = useCallback(
    ...
    [isDragging, viewSize, zoom, pan, clientToSvg, setZoom, setPan],
  )
              ...
              <g key={`label-${group.key}`}>
                ...
                {showBtn && (
                  <g
                    ...
                    onClick={() => (isOwned ? setPaintToRemove(rep) : toggleOwned(rep.id))}>
                    <rect
