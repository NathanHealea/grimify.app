...
export function matchesSchemeFilter(
...
export function isGroupDimmed(group: PaintGroup, filters: GroupFilterParams): boolean {
  ...
  const matchesSearch =
    filters.searchMatchIds.size === 0 || group.paints.some((p) => filters.searchMatchIds.has(p.id))
...
export function isGroupSchemeDimmed(
  group: PaintGroup,
  isSchemeMatching: (p: ProcessedPaint) => boolean,
): boolean {
